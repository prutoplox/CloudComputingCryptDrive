...
namespace Cryptdrive
{
    class DropBoxBase
    {
        ...
        public DropBoxBase(string ApiKey, string ApiSecret, string ApplicationName = "CryptDriveSS2018")
        {
            try
            {
                AppKey = ApiKey;
        ...
        public string GenerateAccessToken()
        {
            try
            {
                ...
                if (CanAuthenticate())
                {
                    ...
                    DropBoxLoginForm login = new DropBoxLoginForm(AuthenticationURL, this.oauth2State); // WPF window with Webbrowser control to redirect user for Dropbox login process.
                    login.ShowDialog();
                    if (login.Result)
                    {
